...
const RecurringList = () => {
  ...
  useEffect(() => {
    const fetchRecurringData = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/departments/${currentUser.department_id}/dashboard/recurring-expense-list`);
        if (!response.ok) {
          throw new Error(`API request failed with status ${response.status}`);
        }
        const data = await response.json();
        console.log("this is recurring list data:",data)
  ...
  const filteredData = recurringData.filter((item) => {
    const matchesTab =
      ...
      item.approvalStatus.toLowerCase() === activeTab.toLowerCase();
    ...
    const matchesSearch =
      ...
      item.expenseInterval.toLowerCase().includes(searchValue) ||
      item.assignedTo.toString().includes(searchValue);
  ...
  const handleViewBudget = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/recurringexpenses/${selectedBudget.id}`);


      if (!response.ok) {
        throw new Error(`Failed to fetch recurring file: ${response.statusText}`);
      }
      ...
      a.href = url;
      a.download = `recurring_${selectedBudget.id}.csv`;
      ...
      console.error("Error downloading recurring file:", error);
      alert("Failed to download recurring file. Please try again.");
    }
  };

  return (
    <div className="container my-5 budgetlist budgetlistadvanced">
      {/* Search and Create Expense Section */}
          ...
          <table className="table">
            ...
            <tbody>
              {currentItems.map((item, index) => (
                <tr key={index}>
                  <td className="budget-name">{item.name}</td>
                  <td className="budget-amount text-right">
                    ₦ {item.amount.toLocaleString()}
                  </td>
                  <td className="text-truncate" style={{ maxWidth: "100px" }}>
                    {item.expenseInterval}
                  </td>
                  ...
                  <td>
                    <span
                      className={`status-badge text-${item.approvalStatus.toLowerCase()}`}
                    >
                      {item.approvalStatus}
                    </span>
                  </td>
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                <div className="modal-body text-center p-4">
                  ...
                  <h3 className="mb-2">View Expense?</h3>
